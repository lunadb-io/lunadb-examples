...
import {
  v0betaCreateDocument,
  v0betaDeleteDocument,
  v0betaGetDocumentContent,
  v0betaSyncDocument,
...
app.post("/doc", async (req, res) => {
  ...
  try {
    ...
    try {
      await v0betaSyncDocument(process.env.LUNADB_HOST, key, "0", [
        {
          op: "insert",
          pointer: "/doc",
          content: "",
      ...
      console.log(
        "Failed to create initial document contents. Document should be recreated!",
        e
      );
      res.status(500).end();
      return;
  ...
  try {
    const resp = await v0betaGetDocumentContent(
